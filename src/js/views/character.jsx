...
export const Character = () => {
  ...
  useEffect(() => {
    async function getCharacters() {
      const response = await fetch(`https://swapi.tech/api/people/${char_id}`);
  ...
  return (
    <div className="container-fluid d-flex">
      ...
      <div className="header position-fixed-top ml-auto ps-2">
        <h1 className="display-1 text-center">{character.name}</h1>
        <p>
        ...
        <span className="d-flex flex-row text-danger justify-content-sm-between">
          <h4>
            Height: <br />
            {character.height}
          </h4>
